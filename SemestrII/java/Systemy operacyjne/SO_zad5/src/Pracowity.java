...
public class Pracowity {
    ...
    public Pracowity(Symulacja symulacja) {
    ...
    private boolean przepiszZadanie(int indeksProc, Zadanie zadanie) {
        ...
        if (biezacy.getObciazenie() + zadanie.getObciazenie() <= biezacy.getProgMax()) {
        ...
        } else { //(biezacy.getObciazenie() + zadanie.getObciazenie() >= biezacy.getProgMax()) {
            while (licznik < pomocnicza.length - 1 && !czyPrzepisano) {                              //sprawdzamy czy nie sprawdziliśmy wszystkich procesorów
        ...
        if (!czyPrzepisano) {                                                                    //sprawdzamy czy został osiągnięty limit na bieżacym procesorze
            ...
            } else {                                                                                      //dodanie do kolejki jeśli wszyscy zajęci
